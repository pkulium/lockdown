...
if __name__ == '__main__':
                ...
                for anp_steps in [1]:
                    for anp_alpha in [0.2]:
                        for round in [10]:
                            ...
                            id2mask_values[-1] = torch.tensor([mask_values[i][3] for i in range(len(mask_values))])
                            # print('-' * 64)
    ...
    for rnd in tqdm(range(1, 2)):
        ...
        for agent_id in chosen:
            ...
            id2mask_values[agent_id] = torch.tensor([mask_values[i][3] for i in range(len(mask_values))])
            print('-' * 64)
            print(f'|settings: {mask_lr}, {anp_eps}, {anp_steps}, {anp_alpha}, {round} |')
            with torch.no_grad():
                val_loss, (val_acc, val_per_class_acc), _ = utils.get_loss_n_accuracy(local_model, criterion, val_loader,args, rnd, num_target)
